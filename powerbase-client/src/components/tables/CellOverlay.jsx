...
class ReactDataViewer extends React.Component {
  ...
  propagateState(state, callback = _.noop) {
    ...
    if (_.has(state, "columns") && !_.get(state, "formattingUpdate", false)) {
      state.columns = gu.updateColWidths(
        this.state,
        state,
        settings,
        maxColumnWidth
    ...
    this.setState(
      _.omit(state, ["formattingUpdate", "renameUpdate", "triggerBgResize"]),
      callback
    );
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.state.triggerResize) {
      const styles = gu.buildGridStyles(
        this.props.theme,
        gu.getRowHeight(0, this.state, this.props)
      );
    ...
    if (
      _.sum(_.map(gu.getActiveCols(this.state), "width")) !=
      _.sum(_.map(gu.getActiveCols(prevState), "width"))
    ) {
  ...
  getData(ids, refresh = false) {
    ...
    if (!refresh) {
      newIds = gu.getRanges(
        _.filter(_.range(ids[0], ids[1] + 1), (i) => !_.has(data, i))
      );
      ...
      if (currGrid) {
        const currGridIds = _.range(
          currGrid._renderedRowStartIndex,
          currGrid._renderedRowEndIndex
        );
    ...
    const params = buildURLParams(
      { ...this.state, ...this.props.settings, ids: newIds },
      URL_PROPS
    );
    if (_.isEmpty(params)) {
      ...
      .then((data) => {
        ...
        if (_.isEmpty(columns)) {
          newState.columns = _.map(data.columns, (c) => ({
            locked:
              c.name === gu.IDX || _.includes(settings?.locked ?? [], c.name),
            ...gu.calcColWidth(c, {
              ...this.state,
              ...newState,
              ...settings,
  ...
  _onSectionRendered({
    ...
    const newIds = _.difference(
      _.range(rowStartIndex, rowStopIndex + 1),
      _.range(ids[0], ids[1] + 1)
    );
      ...
      >
        ...
        >
          {({ onRowsRendered }) => {
            ...
            return (
              <AutoSizer
                className="main-grid col p-0"
                onResize={() => this._grid.recomputeGridSize()}
              >
                {({ width, height }) => (
                  <>
                    <RibbonMenu />
                    ...
                    <MultiGrid
                      ...
                      columnWidth={({ index }) =>
                        gu.getColWidth(index, this.state, this.props)
                      }
                      rowHeight={({ index }) =>
                        gu.getRowHeight(index, this.state, this.props)
                      }
