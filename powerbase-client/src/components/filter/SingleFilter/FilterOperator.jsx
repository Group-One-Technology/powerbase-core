  ...
  return (
    <Listbox value={value} onChange={onChange}>
      <div className="block w-full">
        ...
        <Listbox.Options className="absolute z-10 mt-1 w-auto bg-white shadow-lg max-h-60 rounded-md py-1 text-sm ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none min:w-48 sm:text-sm">
          {options?.map((option) => (
            <Listbox.Option
              key={option}
              value={option}
