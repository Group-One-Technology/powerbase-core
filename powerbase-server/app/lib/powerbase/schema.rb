...
class Powerbase::Schema
  ...
  def create_role(role_name, create_db: false, create_role: false, inherit: false)
    ...
    Sequel.connect(ENV["POWERBASE_CLOUD_DB_CONNECTION"]) {|db|
  ...
  def create_database
    ...
    Sequel.connect(ENV["POWERBASE_CLOUD_DB_CONNECTION"]) do |db|
  ...
  def drop_database(database_name, username = nil)
    begin
      ...
      raise ex if !ex.message.include?(" database \"#{database_name}\" does not exist")
    end
  end
end
